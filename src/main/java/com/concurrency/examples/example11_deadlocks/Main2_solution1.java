...
public class Main2_solution1 {
	...
	private static class Thread2 extends Thread {
		public void run() {
			synchronized (lock1) {
				System.out.println(ThreadColor.ANSI_RED + "Thread 2: Has lock1");
				try {
					Thread.sleep(1000);
				} catch (InterruptedException e) {

				}
				System.out.println(ThreadColor.ANSI_RED + "Thread 2: Waiting for lock2");
				synchronized (lock2) {
					System.out.println(ThreadColor.ANSI_RED + "Thread 2: Has lock1 and lock2");
				}
				System.out.println(ThreadColor.ANSI_RED + "Thread 2: Realased lock2");
			}
			System.out.println(ThreadColor.ANSI_RED + "Thread 2: Realised lock1. Exiting...");
		}
	}
